...
我们使用 singleflight 来防止缓存击穿，具体实现思路：

- 使用 map 缓存对 key 的请求
- 使用 sync.WaitGroup 控制并发

当同一时刻有多个相同的请求时(且未命中时)，只有第一个请求才会创建真实的数据库请求，其他的请求只需通过 WaitGroup 等待这个请求完成即可。

## 使用 Protobuf 通信

使用 Protocol Buffer 来进行通信，替代 HTTP，因为远程节点直接的通信是很简单的，并不需要传输很复杂的信息，使用 Protobuf 的二进制传输和方便的结构体编解码 要比 HTTP 更好。

作者这一节并不是完全的 Protobuf，在评论区看到了一个使用 grpc 实现的版本，感觉那个要更合适一点。
